...
# -- they restate `comments` and `reviews` in full, and when they were kept they
# added 2113 redundant events against the 495 that say something.
TIMELINE_EVENTS = {
    "cross-referenced",
    "referenced",
    "connected",
...
def write_index(manifest: dict, check: bool) -> None:
    ...
    add("")
    add("Generated by `sync.py` -- edit `sources.json`, not this file. Every")
    add("entry links its source and names the `sources.json` entries that")
    add("reached it; `why` there is the standing answer to what an entry is")
    add("doing in the corpus.")
    add("")
    ...
    if manifest.get("unfetched"):
        add("## Not fetched")
        add("")
        if manifest.get("unfetched_why"):
            add(manifest["unfetched_why"])
            add("")
