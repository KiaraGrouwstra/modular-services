...
#
# Derived from `modules/default.nix` and `modules/finit/system.nix` in
# finix-modular-services
# (https://github.com/DigitalBrewStudios/finix-modular-services). This version
# uses the portable layer of this repository, puts `configData` in
# `environment.etc` and declares the variant registry `modularServices`.
{
  lib,
  config,
  options,
...
let
  ...
  makeServices =
    ...
    concatMapAttrs (name: module: {
      # finix declares `finit.services` with `types.submodule`, which reads an
...
in
{
  ...
  options = {
    system.services = mkOption {
      description = ''
        A collection of [modular services](https://nixos.org/manual/nixos/unstable/#modular-services) that are configured as finit services.
      '';
    ...
    modularServices = mkOption {
      ...
      description = ''
        finix variants of the modular services, keyed by `<pkg>.<service>`.
