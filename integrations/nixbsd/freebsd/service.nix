...
#
# Copied from `modules/system/service/freebsd/service.nix` in NixBSD
# (https://github.com/nixos-bsd/nixbsd).
#
# TODO: the main rc service runs the program through `daemon -P`, which
# writes the PID of the supervisor and not of the program. Thus signals from
# rc.subr go to `daemon`, and `process.reloadSignal` has no effect.
# FreeBSD rc.subr also has no readiness protocol, thus `notificationProtocol`
# has no effect here.
