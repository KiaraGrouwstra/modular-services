# Run:
#   nix build .#checks.<system>.nixos-modular-variants
#
# Checks that the service variants registered in ../modular/default.nix keep
# their file attribution. Registering a variant as
# `import ./<pkg>/<svc>/system.nix` instead of `./<pkg>/<svc>/system.nix` yields
# a bare function, which carries no `_file`, so the module system falls back to
# the location where the `deferredModule` was defined and the variant's own file
# is lost.
#
...
let
  ...
  # the pure module under `modular-services/`, and the NixOS-specific variant
  # under `integrations/nixos/modular/`.
  expected = {
    holod = [
      "/integrations/nixos/modular/holo-daemon/default/system.nix"
      "/modular-services/holo-daemon/service.nix"
    ];
    phpfpm = [
      "/integrations/nixos/modular/php/default/system.nix"
      "/modular-services/php/service.nix"
    ];
  };
