...
| `integrations/nixos/modular/<pkg>/<svc>/system.nix` | `nixos/modules/system/service/modular/<pkg>/<svc>/system.nix` | verbatim | The systemd half of each variant, holding what the service modules below no longer do. `easytier` and `redlib` have no upstream counterpart. `easytier`, like `python-http-server`, defines nothing, and exists so that every service in `modular-services/` is registered; `redlib` holds the systemd definitions that nixpkgs pull request 475372 still keeps in the service module. |
...
| `modular-services/redlib/service.nix` | `pkgs/by-name/re/redlib/service.nix` | modified | Takes an empty non-module dependency set, so it is reached through `importApply` like every other service here. The `package` option is the plain `mkOption` shape the other services use, rather than `lib.options.mkModularPackageOption`, which is not in the pinned `lib`. The systemd definitions move to `integrations/nixos/modular/redlib/default/system.nix`. |
