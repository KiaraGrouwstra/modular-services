...
| `integrations/nixos/systemd/user/default.nix` | `nixos/modules/system/service/systemd/user/default.nix` | modified | The portable-layer import becomes `../../../../lib/services`, as in `../system/default.nix`. The integration passes `defaultWantedBy = [ "default.target" ]` rather than importing an override module. The unit files go in the profile of the user rather than in `systemd.user.services`, so that the unit of one user is not visible to, and startable by, all of them. |
