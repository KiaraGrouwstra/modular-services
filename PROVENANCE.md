...
| `integrations/nixos/modular/php/default/system.nix` | `nixos/modules/system/service/modular/php/default/system.nix` | modified | Carries the reload switch of nixpkgs pull request 434262: `Type = "notify-reload"` with `ReloadSignal = "USR2"` when `systemd_interval` is non-zero, and the `Type = "notify"` plus `ExecReload` pair otherwise. Upstream still has that switch in the service module, from which the systemd definitions had not yet moved when the pull request was written. |
...
| `modular-services/php/service.nix` | `pkgs/development/interpreters/php/service.nix` | modified | Test-path comment retargeted to `integrations/nixos/tests/packages/php-fpm.nix`. The systemd definitions move to `integrations/nixos/modular/php/default/system.nix`, which takes `coreutils` with them, so the service's non-module dependencies narrow to `{ formats }`. `systemd_interval` defaults to 10 per nixpkgs pull request 434262, which also sets `nohup` in a finit block this copy no longer has. |
