...
| `doc/design/` | Working material rather than manual: the design as it was argued, not as it settled. [`crowdsource/`](./doc/design/crowdsource/README.md) is the discussion itself -- issues, RFCs, meeting minutes, the Matrix room and the project board -- fetched from where it happened and kept current by `nix run .#crowdsource-sync`. |
