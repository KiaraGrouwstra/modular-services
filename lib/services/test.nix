# Pure-eval test for the portable layer. It takes `lib` as a parameter rather
# than importing one, so it can run against whichever `lib` the consumer pins.
#
# Run:
#   nix build .#checks.<system>.lib-eval
{ lib }:
